...
class FrictionForRectangle{
private:
    double k;
public:
    FrictionForRectangle(double k){
        this->k = k;
    }
    void force_torque_gen(Rectangle& rec){
        ///rec.force_TOT += -k * rec.vel_CM;

        Vector2D vel_Center;

        Vector2D e_n = rec.pos_Center - rec.pos_CM;
        e_n.normalize();

        Vector2D e_t = e_n;
        e_t.turn(pi/2.0);

        vel_Center = rec.vel_CM + rec.ang_acc * (rec.pos_Center - rec.pos_CM).len() * e_t;



        rec.torque_TOT += -k * (rec.pos_Center - rec.pos_CM)^(vel_Center) * abs( (rec.pos_Center - rec.pos_CM)^(rec.vel_CM) ) / (rec.pos_Center.len() * rec.pos_CM.len() );
    }
};
...
{
    return (int)pow((double)i, 4.0);
}
int main()
{
    sf::RenderWindow window(sf::VideoMode(800, 700), "SFML works!",sf::Style::Default);
    ...
    double speed = 80.0;
    double theta = pi/2.3;
    Rectangle rec(100.0, 100.0, speed*cos(theta), speed*sin(theta), theta, 0.0, 40.0, 4.0,distr);

    ///Rectangle rec(500.0, 400.0, 0.0, 20.0, pi/6.0, 0.0, 80.0, 8.0,distr);

    UniformGravityForRectangle rec_uni_grav;
    FrictionForRectangle fric_for_rec(1.00 * 300);


    BlackHoleForRectangle bobo(300.0,300.0,3e5);
    ...
    while (window.isOpen())
    {
        ...
        float duration = clock.getElapsedTime().asSeconds() * 0.50 * 5 ;
        clock.restart();
        int frame = 10;
        ...
        while(frame > 0){
            ...
            **/
            ...
            fric_for_rec.force_torque_gen(rec);
