...
void Player::setSize(int width, int height ){
	//currently kind of useless
    playerWidth = width;
	playerHeight = height;
}

void Player::updateFrame(double time){
	// for now: rotates bow with any positive angle, drag below to rotate the bow upward, arm position is correct, sizing is also correct. 
    ...
    spriteb -> setPosition(float(windowSize.x)/25, float(windowSize.y)/5);      
