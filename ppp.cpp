...
int main()
{
    ...
    Rectangle rec(100.0, 100.0, 60.0, 35.0, pi/6.0, 0.0, 200.0, 10.0,distr);
    ...
    while (window.isOpen())
    {
        ...
        if (sf::Mouse::isButtonPressed(sf::Mouse::Left) && flag == false){
            ...
            ///cout<<x<<" "<<y<<endl;
            Point* point = new Point(20.0, x, y, 60.0, 30.0, 10.0);
            points.push_back(point);
            gravity.add_point(*point);
            CGenerator.add_point(*point);
            flag = true;
        ...
        ///float duration = clock.getElapsedTime().asSeconds() * 4.0;
